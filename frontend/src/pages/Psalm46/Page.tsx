...
export default function Psalm46Page() {
  ...
  useEffect(() => {
    if (section === 'chat' && isLoggedIn) {
      ...
      socketRef.current.on('connect', () => {
        ...
        if (userName) {
          socketRef.current.emit('set-username', { username: userName, group: 'psalm46' })
          socketRef.current.emit('join-room', 'psalm46-chat')
        }
      })
  ...
  const sendMessage = (e: React.FormEvent) => {
    ...
    if (newMessage.trim() && userName.trim() && socketRef.current) {
      ...
      setNewMessage('')
      setIsTyping(false)
      socketRef.current.emit('user-typing', { user: userName, isTyping: false, room: 'psalm46-chat', group: 'psalm46' })
    }
  }
