...
export default function ChatPage() {
  ...
  const FileAttachment = ({ fileAttachment, isOwn }: { fileAttachment: any, isOwn: boolean }) => {
    ...
    const handlePreview = () => {
      if (fileAttachment.fileType === 'image' || fileAttachment.fileType === 'video') {
      ...
      <div className="flex items-center space-x-3 py-2">
        ...
        <div className="flex-1">
          ...
          >
            <div 
              className={`absolute top-0 left-0 h-full rounded-full transition-all duration-100 ${
                isOwn ? 'bg-white/30' : 'bg-green-500'
              }`}
            ...
            <div className="absolute inset-0 flex items-center justify-center space-x-0.5 px-2">
              {voiceNote?.waveform?.slice(0, 20).map((height: number, index: number) => (
                <div
                  key={index}
                  ...
                  style={{ height: `${Math.max(4, height / 10)}px` }}
                />
              ))}
            </div>
          </div>
          ...
          <div className={`flex justify-between text-xs mt-1 ${
            ...
            <span>{formatDuration(voiceNote?.duration || 0)}</span>
          </div>
        </div>
      </div>
    )
