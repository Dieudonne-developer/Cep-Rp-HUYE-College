...
export default function ChoirPage() {
  ...
  const getYouTubeEmbedUrl = (url: string) => {
    try {
      ...
      if (url.includes('youtube.com/watch?v=')) {
        // Extract video ID from watch URL, removing playlist parameters
  ...
  useEffect(() => {
    // Initialize section from query param
    const params = new URLSearchParams(location.search)
    const sectionParam = (params.get('section') || '').toLowerCase()
    const allowed: Section[] = ['about','media','activities','ideas','support','chat']
    if (allowed.includes(sectionParam as Section)) {
  ...
  const fetchSupportData = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${baseUrl}/api/home/support/choir`)
  ...
  useEffect(() => {
    if (section === 'media') {
      // Fetch songs from public API
      fetch(`${baseUrl}/api/choir/songs`).then(r=>r.json()).then(setSongs).catch(()=>{})
    } else if (section === 'activities') {
      // Fetch events from public API
      fetch(`${baseUrl}/api/choir/events`).then(r=>r.json()).then(events => {
      ...
      fetch(`${baseUrl}/api/choir/implemented-ideas`).then(r=>r.json()).then(r=> setIdeas(r.data||[])).catch(()=>{})
    } else if (section === 'support') {
      fetchSupportData()
    }
  }, [section, baseUrl, fetchSupportData])

  // Keep URL query in sync when switching tabs (so header links work and sharable URLs exist)
  useEffect(() => {
    const params = new URLSearchParams(location.search)
    if ((params.get('section') || '') !== section) {
      params.set('section', section)
      navigate({ pathname: '/choir', search: params.toString() }, { replace: true })
    }
  ...
  function submitIdea(e: React.FormEvent<HTMLFormElement>) {
    ...
    fetch(`${baseUrl}/api/choir/ideas`, { method:'POST', headers:{ 'Content-Type':'application/json' }, body: JSON.stringify(payload) })
      .then(async (r) => {
        if (!r.ok) {
          let err
        ...
        <div className="max-w-7xl mx-auto">
          {section === 'about' && (
            <div className="p-6 bg-white rounded-lg shadow-md">
              <h2 className="text-3xl font-bold text-gray-800 mb-6 flex items-center"><i data-lucide="info" className="mr-2 text-blue-600" /> About Ishyanga Ryera Choir</h2>
              <div className="grid md:grid-cols-3 gap-6">
                <div className="md:col-span-2">
                  <div className="text-gray-700 leading-relaxed space-y-4">
                    <p>Ishayangaryera Choir is a gospel choir that serves God through music within CEP RP Huye (Communauté des Étudiants Protestants at RP Huye). It is a group of devoted Christians who have dedicated their musical talents to spreading the Good News of Jesus Christ through songs of praise, thanksgiving, and worship that inspire hearts to draw closer to their Creator. Ishayangaryera Choir has faithfully served in ministry for years, helping believers and listeners understand that worship is a way to commune with God and to express gratitude for His works in our lives.</p>
                    
                    <p>The members of this choir are students and members of CEP RP Huye who share a common mission of serving God through their voices, their compositions, and their fellowship as brothers and sisters in faith. Their songs touch hearts, comfort the weary, strengthen the weak, and encourage many to continue walking the path of salvation.</p>
                    
                    <p>Ishayangaryera Choir is known for its excellence, dedication, and unity in musical performance, qualities that make its songs stand out and loved by audiences both in Rwanda and beyond. Their musical works can be found on their official YouTube channel, "ISHYANGA RYERA CHOIR CEP IPRC HUYE", where they share videos and recordings of their latest and most popular songs.</p>
                    
                    <p>Among their well-known songs are "Umusaraba wa Yesu" (The Cross of Jesus) and "Umusaraba" (The Cross)—both powerful pieces that express God's love revealed through the sacrifice of Jesus Christ on the cross. Their heartfelt voices and uplifting messages strengthen the choir's mission: to proclaim to the world that Jesus Christ is the Way, the Truth, and the Life.</p>
                    
                    <p>Ishayangaryera Choir continues its ministry of sharing the Gospel through music, prayer, and acts of love. The choir envisions growing both spiritually and artistically, and warmly invites everyone to follow their work on YouTube, to pray for them, and to support their mission so that the Good News of Jesus Christ may reach even further.</p>
                  </div>
                  
                  <h3 className="text-2xl font-semibold text-gray-700 mb-3 mt-6">Leadership</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="bg-blue-50 p-4 rounded-lg"><h4 className="font-bold text-blue-800">Choir Director</h4><p className="text-gray-700">Pastor Jean de Dieu Niyonzima</p></div>
                    <div className="bg-blue-50 p-4 rounded-lg"><h4 className="font-bold text-blue-800">Assistant Director</h4><p className="text-gray-700">Alice Uwase</p></div>
                    <div className="bg-blue-50 p-4 rounded-lg"><h4 className="font-bold text-blue-800">Secretary</h4><p className="text-gray-700">Eric Mugisha</p></div>
                    <div className="bg-blue-50 p-4 rounded-lg"><h4 className="font-bold text-blue-800">Treasurer</h4><p className="text-gray-700">Grace Ingabire</p></div>
                  </div>
                </div>
                <div className="bg-blue-50 p-6 rounded-lg">
                  <h3 className="text-xl font-semibold text-blue-800 mb-4">Quick Facts</h3>
                  <ul className="space-y-3">
                    <li className="flex items-start"><i data-lucide="users" className="mr-2 text-blue-600 mt-1 flex-shrink-0" /><span><strong>45 members</strong> (students and alumni)</span></li>
                    <li className="flex items-start"><i data-lucide="music" className="mr-2 text-blue-600 mt-1 flex-shrink-0" /><span><strong>120+ songs</strong> in our repertoire</span></li>
                  </ul>
                </div>
              </div>
            </div>
          ...
          {section === 'media' && (
            <div className="p-6 bg-white rounded-lg shadow-md">
              <h2 className="text-3xl font-bold text-gray-800 mb-6 flex items-center"><i data-lucide="music" className="mr-2 text-blue-600" /> Videos & Songs</h2>
              ...
              <div className="mb-8">
                <h3 className="text-2xl font-semibold text-gray-700 mb-4">Videos & Songs</h3>
                      ...
                      {song.mediaType === 'video' ? (
                        <div className="mb-3">
                          {isYouTubeUrl(song.url) && !failedVideos.has(song._id) ? (
                            <div className="relative group" style={{paddingBottom:'56.25%',height:0,overflow:'hidden'}}>
                              ...
                              <div className="absolute inset-0 bg-gray-100 rounded-lg flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
                                <div className="text-center p-4">
                                  <p className="text-sm text-gray-600 mb-2">Video not loading?</p>
                                  <button
                            ...
                            // Fallback for failed YouTube videos
                                ...
                                <div className="text-center p-4">
                                  ...
                                  <div className="space-y-2">
                                    ...
                                    <button
                                      ...
                                      className="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg text-sm transition block w-full"
                                    >
                                      Retry Loading
                                    </button>
                                  </div>
              ...
              {/* Photo gallery removed - focusing on admin-uploaded content only */}
            </div>
          )}

          {section === 'activities' && (
            <div className="p-6 bg-white rounded-lg shadow-md">
              <h2 className="text-3xl font-bold text-gray-800 mb-6 flex items-center"><i data-lucide="calendar" className="mr-2 text-blue-600" /> Choir Activities</h2>
              <div className="mb-8">
          ...
          {section === 'support' && (
            <div className="p-6 bg-white rounded-lg shadow-md">
              <h2 className="text-3xl font-bold text-gray-800 mb-6 flex items-center"><i data-lucide="heart-handshake" className="mr-2 text-blue-600" /> Support Our Ministry</h2>
              {loadingSupport ? (
              ...
              {!isLoggedIn ? (
                <div className="max-w-md mx-auto">
                  <div className="bg-blue-50 p-6 rounded-lg">
                    ...
                    <p className="text-gray-700 mb-4">Please login to access the choir chat.</p>
                      ...
                      onSubmit={async (e) => {
                        ...
                        try {
                          ...
                          if (data.success) {
                            ...
                            sessionStorage.setItem('userGroup', data.user.userGroup || 'choir')
                            setUserName(data.user.username)
                            setIsLoggedIn(true)
                            window.location.href = '/choir/chat'
                          } else {
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between text-sm">
                        <button
                          type="button"
                          onClick={() => window.location.href = '/choir/forgot-password'}
                        ...
                        <button
                          type="button"
                          onClick={() => window.location.href = '/choir/register'}
                          className="text-blue-700 hover:text-blue-900"
                        >
                          Register
                        </button>
                ...
                <div className="max-w-4xl mx-auto">
                  <div className="bg-blue-50 p-6 rounded-lg text-center">
                    ...
                    <button onClick={() => (window.location.href = '/choir/chat')} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg">Go to Chat</button>
                  </div>
                </div>
              )}
            </div>
