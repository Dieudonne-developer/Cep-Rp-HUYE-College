...
type Section = 'about'|'activities'|'ideas'|'support'|'chat'
...
export default function SocialPage() {
  ...
  useEffect(() => {
    if (section === 'chat' && isLoggedIn) {
      ...
      socketRef.current.on('connect', () => {
        ...
        if (userName) {
          socketRef.current.emit('set-username', { username: userName, group: 'social' })
          socketRef.current.emit('join-room', 'social-chat')
        }
      })
  ...
  const sendMessage = (e: React.FormEvent) => {
    ...
    if (newMessage.trim() && userName.trim() && socketRef.current) {
      ...
      setNewMessage('')
      setIsTyping(false)
      socketRef.current.emit('user-typing', { user: userName, isTyping: false, room: 'social-chat', group: 'social' })
    }
  }
    ...
    <div className="min-h-screen bg-gray-100 text-gray-900 flex flex-col">
      ...
      <nav className="bg-blue-800 text-white shadow-md py-2 sticky top-16 z-40">
        <div className="container mx-auto px-4">
          <div className="flex flex-wrap justify-center md:justify-start space-x-1 md:space-x-4">
            {(['about','activities','ideas','support','chat'] as Section[]).map((s) => (
              <button key={s} onClick={() => setSection(s)} className={`px-3 py-2 rounded-lg transition duration-300 ease-in-out hover:bg-blue-700 flex items-center ${section===s?'bg-blue-900':''}`}>
                <i data-lucide={s==='about'?'info': s==='activities'?'calendar': s==='ideas'?'lightbulb': s==='support'?'heart-handshake':'message-circle'} className="mr-2 w-4 h-4" /> {s[0].toUpperCase()+s.slice(1)}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto">
          {section === 'about' && (
            <div className="p-6 bg-white rounded-lg shadow-md">
              <h2 className="text-3xl font-bold text-gray-800 mb-6 flex items-center"><i data-lucide="info" className="mr-2 text-blue-600" /> About Social Family</h2>
              <div className="grid md:grid-cols-3 gap-6">
                <div className="md:col-span-2">
                  <div className="text-gray-700 leading-relaxed space-y-4">
                    <p>Social Family is a fellowship within CEP RP Huye whose mission is to serve the community through social activities that reflect the love of Christ in action. It is a group of compassionate individuals dedicated to helping the less fortunate, the sick, and those facing various life challenges. Social Family engages in acts of kindness such as visiting patients, supporting vulnerable families, providing material assistance, and offering spiritual encouragement—all inspired by Christian love.</p>
                    
                    <p>Members of this fellowship believe that helping others is one of the greatest ways to demonstrate God's love at work within His people. They have committed themselves to be the hands of Christ on earth—serving God through helping others and restoring hope in their lives. The group also prays for those going through difficulties, showing them that they are not alone in their struggles.</p>
                    
                    <p>Social Family is characterized by love, unity, and a strong commitment to serving God through practical deeds. They uphold the Word of God, which calls believers to love others as Christ loved them, and they strive to live out that command daily through their acts of kindness. The fellowship's ultimate goal is to continue promoting Christian-based social welfare activities so that those who are suffering or in distress may find hope and feel God's care and presence in their lives.</p>
                  </div>
                ...
                <div className="bg-blue-50 p-6 rounded-lg">
                  ...
                  <ul className="space-y-3">
                    <li className="flex items-start"><i data-lucide="users" className="mr-2 text-blue-600 mt-1 flex-shrink-0" /><span><strong>Vibrant community</strong> focused on fellowship and connection</span></li>
                    <li className="flex items-start"><i data-lucide="heart-handshake" className="mr-2 text-blue-600 mt-1 flex-shrink-0" /><span><strong>Regular events</strong> for socializing, outreach, and celebration</span></li>
                  </ul>
                </div>
              </div>
            </div>
              ...
              {!isLoggedIn ? (
                  ...
                  <div className="bg-blue-50 p-6 rounded-lg">
                      ...
                      onSubmit={async (e) => {
                        ...
                        try {
                          ...
                          if (data.success) {
                            ...
                            sessionStorage.setItem('userGroup', data.user.userGroup || 'social')
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between text-sm">
                        <button
                          type="button"
                          onClick={() => window.location.href = '/social/forgot-password'}
                        ...
                        <button
                          type="button"
                          onClick={() => window.location.href = '/social/register'}
                          className="text-blue-700 hover:text-blue-900"
                        >
                          Register
                        </button>
                ...
                <div className="max-w-4xl mx-auto">
                  <div className="bg-blue-50 p-6 rounded-lg text-center">
                    ...
                    <button onClick={() => (window.location.href = '/social/chat')} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg">Go to Chat</button>
                  </div>
                </div>
              )}
            </div>
      ...
      <footer className="bg-gray-800 text-white p-4 text-center mt-8">
        <div className="container mx-auto">
          ...
          <p className="text-sm mt-2">Social Family - Building relationships and fostering fellowship</p>
        </div>
      </footer>
